...
public class GermplasmBaseResource extends PaginatedServerResource
{
	...
	protected SelectJoinStep<Record1<Integer>> getGermplasmIdQueryWrapped(DSLContext context)
	{
		Germinatebase g = GERMINATEBASE.as("g");
		...
		return context.selectDistinct(DSL.field(GERMPLASM_ID, Integer.class)).from(context.select(fields)
					  .from(GERMINATEBASE)
					  .leftJoin(g).on(g.ID.eq(GERMINATEBASE.ENTITYPARENT_ID))
					  .leftJoin(INSTITUTIONS).on(INSTITUTIONS.ID.eq(GERMINATEBASE.INSTITUTION_ID))
					  .leftJoin(ENTITYTYPES).on(ENTITYTYPES.ID.eq(GERMINATEBASE.ENTITYTYPE_ID))
	...
	protected SelectJoinStep<?> getGermplasmQueryWrapped(DSLContext context) {
		Germinatebase g = GERMINATEBASE.as("g");
		...
		return select.from(context.select(fields).from(GERMINATEBASE)
								  .leftJoin(g).on(g.ID.eq(GERMINATEBASE.ENTITYPARENT_ID))
								  .leftJoin(INSTITUTIONS).on(INSTITUTIONS.ID.eq(GERMINATEBASE.INSTITUTION_ID))
								  .leftJoin(ENTITYTYPES).on(ENTITYTYPES.ID.eq(GERMINATEBASE.ENTITYTYPE_ID))
								  .leftJoin(TAXONOMIES).on(TAXONOMIES.ID.eq(GERMINATEBASE.TAXONOMY_ID))
	...
	protected SelectJoinStep<?> getGermplasmQuery(DSLContext context, Field<?>... additionalFields)
	{
		Germinatebase g = GERMINATEBASE.as("g");
