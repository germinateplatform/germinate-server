...
import static jhi.germinate.server.database.codegen.tables.Grouptypes.GROUPTYPES;
...
public class DatasetGroupResource extends ContextResource
{
		...
		try (Connection conn = Database.getConnection())
		{
			...
			switch (request.getDatasetType())
			{
				...
				case "trials":
					...
					break;
				case "pedigree":
					resultStep = getPedigreeGroups(step, requestedIds, userDetails.getId());
					break;
				case "genotype":
				case "allelefreq":
	...
	private SelectConditionStep<? extends Record> getPedigreeGroups(SelectSelectStep<? extends Record> step, List<Integer> requestedIds, Integer userId)
	{
		return step.from(GROUPS)
		           .leftJoin(GROUPTYPES).on(GROUPTYPES.ID.eq(GROUPS.GROUPTYPE_ID))
		           .leftJoin(GROUPMEMBERS).on(GROUPMEMBERS.GROUP_ID.eq(GROUPS.ID))
		           ...
		           .and(GROUPS.VISIBILITY.eq(true).or(GROUPS.CREATED_BY.eq(userId)))
		           .andExists(DSL.selectOne().from(PEDIGREES)
		                         .where(PEDIGREES.GERMINATEBASE_ID.eq(GROUPMEMBERS.FOREIGN_ID))
		                         .and(GROUPMEMBERS.GROUP_ID.eq(GROUPS.ID))
		                         .and(PEDIGREES.DATASET_ID.in(requestedIds)));
	}
