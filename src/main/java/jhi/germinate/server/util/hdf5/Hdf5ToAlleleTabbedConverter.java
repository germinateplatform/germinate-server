 ...
 */
public class Hdf5ToAlleleTabbedConverter extends AbstractHdf5Converter
{
	private boolean transposed;

	public Hdf5ToAlleleTabbedConverter(Path hdf5File, Set<String> lines, Set<String> markers, Map<String, String> germplasmNameMapping, Path outputFilePath, boolean transposed)
	{
			 ...
			 IHDF5Reader reader = HDF5Factory.openForReading(hdf5File.toFile()))
		{
			...
			if (transposed)
			{
				...
				for (String markerName : markers)
				{
					...
					float[][] g = reader.float32().readMatrixBlock(DATA, hdf5Lines.size(), 1, 0, markerInds.get(markerName));
					float[] genotypes = new float[g.length];
					for (int i = 0; i < g.length; i++)
						genotypes[i] = g[i][0];
					writeGenotypeFlatFileString(bw, markerName, genotypes, lineIndices);
				}
			}
			else
			{
				...
				for (String lineName : lines)
				{
					...
					float[] genotypes = reader.float32().readMatrixBlock(DATA, 1, markerInds.size(), lineInds.get(lineName), 0)[0];
					writeGenotypeFlatFileString(bw, germplasmNameMapping.get(lineName), genotypes, markerIndices);
				}
			}
	...
	private void writeGenotypeFlatFileString(BufferedWriter bw, String lineName, float[] genotypes, List<Integer> markerIndices)
		throws IOException
	{
