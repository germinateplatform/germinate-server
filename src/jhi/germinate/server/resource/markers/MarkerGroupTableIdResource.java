 ...
 */
public class MarkerGroupTableIdResource extends PaginatedServerResource implements FilteredResource
{
	...
	public PaginatedResult<List<Integer>> getJson(PaginatedRequest request)
	{
		...
		{
			SelectJoinStep<Record> from = GroupResource.prepareQuery(getRequest(), context, groupId, VIEW_TABLE_MARKERS_GROUPS, VIEW_TABLE_MARKERS_GROUPS.GROUP_ID, this);

			// Filter here!
			filter(from, filters);

			List<Integer> result = setPaginationAndOrderBy(from)
				.fetch(VIEW_TABLE_MARKERS_GROUPS.MARKER_ID);
